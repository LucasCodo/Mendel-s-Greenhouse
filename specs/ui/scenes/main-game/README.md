---
title: Main Game Scene Wiki
tags:
  - ui/scenes/main-game
  - obsidian/wiki
type: scene
project: mendels-greenhouse
status: draft
aliases:
  - Main Scene
  - Scene Principal
---

# Main Game Scene

The Main Game Scene is the primary playable workspace for Mendel's Greenhouse.
It combines contract tracking, parent selection, genetic analysis, the
Germination Bed, specimen actions, contextual help, and scene navigation.

## Reference Notes

- [[visual-target|Visual Target]]
- [[layout-reference|Layout Reference]]
- [[component-map|Component Map]]

## Components

- [[components/top-bar|Top Bar]]
- [[components/contract-banner|Contract Banner]]
- [[components/genetic-analyzer-panel|Genetic Analyzer Panel]]
- [[components/parent-cross-panel|Parent Cross Panel]]
- [[components/germination-bed|Germination Bed]]
- [[components/specimen-inspection-overlay|Specimen Inspection Overlay]]
- [[components/help-panel|Help Panel]]
- [[components/right-navigation-rail|Right Navigation Rail]]
- [[components/greenhouse-background|Greenhouse Background]]

## Related Specs

- [[specs/ui/screens/main-game-screen|Main Game Screen Spec]]
- [[specs/ui/component-library|Component Library]]
- [[specs/ui/assets/ui-elements|UI Elements]]
- [[specs/mechanics/gameplay|Gameplay Rules]]
